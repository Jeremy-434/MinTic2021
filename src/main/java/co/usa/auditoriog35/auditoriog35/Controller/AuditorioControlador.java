...
public class AuditorioControlador {
    ...
    @PutMapping("/update")
    @ResponseStatus(HttpStatus.CREATED)
    public Auditorio actualizarAuditorio(@RequestBody Auditorio auditorio){
        return auditorioServicio.actualizar(auditorio);
    }

    @DeleteMapping("/{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public boolean borrarAuditorio(@PathVariable("id") int auditorioId){
        return auditorioServicio.borrar(auditorioId);
    } 
